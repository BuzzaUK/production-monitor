...
// Make sure you have this include at the top of your .ino file
#include <Preferences.h> 
// Add any other necessary includes like SPIFFS.h, WiFi.h, WebServer.h, etc.

// Your global variables (like prefs, server, config, assetStates) should be declared here
// Preferences prefs; // Example if it's global
// WebServer server(80); // Example

void setup() {
  Serial.begin(115200);
  if (!SPIFFS.begin(true)) { Serial.println("SPIFFS failed!"); return; }
  prefs.begin("assetmon", false); // Your original prefs.begin()
  loadConfig();
  setupWiFiSmart(); // --- NEW WiFi logic replaces setupWiFi()
  setupTime();
...
// Make sure WiFi.h is included in your sketch for WiFi.status(), WiFi.SSID(), etc.
// #include <WiFi.h> 
...
String htmlConfig() {
  ...
  // Style for the new WiFi reconfigure button
  html += "button.wifi-reconfig{background:#f44336 !important; color:#fff !important;}"; // Added !important to help override general button styles if needed
  ...
  html += "}";
  // JavaScript confirmation for WiFi reconfigure
  html += "function confirmWiFiReconfig(e){";
  html += "if(!confirm('Are you sure you want to enter WiFi setup mode? The device will disconnect from the current network and restart as an Access Point.')){e.preventDefault();return false;}";
  html += "return true;}";
  html += "</script>";
  html += "</head><body>";
  ...
  // --- PATCHED SECTION FOR WIFI RECONFIGURATION ---
  html += "<hr style='margin: 2em 0; border-top: 1px solid #eee;'>"; // Visual separator
  ...
  // Applied a class for specific styling, and used your general button styling as a base
  html += "<button type='submit' class='wifi-reconfig'>Enter WiFi Setup Mode</button>";
  html += "</form>";
  html += "</fieldset>";
