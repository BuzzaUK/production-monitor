...
#include <vector> // --- SIM PATCH --- Add vector include for downtime reasons

struct SimulatedAsset {
  String name;
  bool isRunning;
  unsigned long nextEventTime; // The millis() value when the next event should occur
  String timeToNextEventStr;   // A pre-formatted string for the UI
};

bool simulationEnabled = false; 
SimulatedAsset simulatedAssets[MAX_ASSETS]; 
std::vector<String> simDowntimeReasons;
...
void setup() {
  ...
  // --- SIM PATCH --- START ---
  for (int i = 0; i < config.assetCount; i++) {
    simulatedAssets[i].name = String(config.assets[i].name); 
    simulatedAssets[i].isRunning = true;
  }
  for (int i = 0; i < 5; i++) {
    String reason = String(config.downtimeReasons[i]);
    if (reason.length() > 0) {
      simDowntimeReasons.push_back(reason);
    }
  }
  // --- SIM PATCH --- END ---
 ...
 // =================================================================
// --- Block 2:  Simulation API Endpoints 
// =================================================================

// Endpoint to START the simulation
server.on("/api/simulation/start", HTTP_POST, []() { // No arguments in the lambda
  if (!simulationEnabled) {
    simulationEnabled = true;
    unsigned long currentTime = millis();
    for (int i = 0; i < config.assetCount; i++) {
      // Set initial state to running and log the START event
      simulatedAssets[i].isRunning = true;
      logEvent(i, true, time(nullptr), "Simulation Started"); 
      
      // Schedule the first random STOP
      unsigned long nextInterval = random(60000, 600001); // 1 to 10 minutes
      simulatedAssets[i].nextEventTime = currentTime + nextInterval;
    }
  }
  // Use the global 'server' object to send the response
  server.send(200, "application/json", "{\"status\":\"Simulation started\"}");
});

// Endpoint to STOP the simulation
server.on("/api/simulation/stop", HTTP_POST, []() { // No arguments in the lambda
  simulationEnabled = false;
  // Use the global 'server' object to send the response
  server.send(200, "application/json", "{\"status\":\"Simulation stopped\"}");
});

// Endpoint for the UI to get the current status and countdowns
server.on("/api/simulation/status", HTTP_GET, []() { // No arguments in the lambda
  String json = "{\"enabled\": " + String(simulationEnabled ? "true" : "false") + ", \"assets\": [";
  for (int i = 0; i < config.assetCount; i++) {
    json += "{";
    json += "\"name\": \"" + simulatedAssets[i].name + "\",";
    json += "\"timeToNext\": \"" + simulatedAssets[i].timeToNextEventStr + "\"";
    json += "}";
    if (i < config.assetCount - 1) {
      json += ",";
    }
  }
  json += "]}";
...
void handleSimulation() {
  ...
  // First, loop through and update each asset individually
  for (int i = 0; i < config.assetCount; i++) {
    // Update the countdown string for the UI first
    unsigned long remainingMillis = simulatedAssets[i].nextEventTime > currentTime ? simulatedAssets[i].nextEventTime - currentTime : 0;
    int minutes = (remainingMillis / 1000) / 60;
    int seconds = (remainingMillis / 1000) % 60;
    char buffer[10];
    sprintf(buffer, "%02d:%02d", minutes, seconds);
    simulatedAssets[i].timeToNextEventStr = String(buffer);

    // Now, check if it's time for the next event for this asset
    if (currentTime >= simulatedAssets[i].nextEventTime) {
      AssetState& as = assetStates[i];
      unsigned long elapsed = now_ts - as.lastChangeTime;
      unsigned long runDuration = 0;
      unsigned long stopDuration = 0;
      
      simulatedAssets[i].isRunning = !simulatedAssets[i].isRunning;
      
      if (simulatedAssets[i].isRunning) { // Machine just STARTED (was stopped)
        as.stoppedTime += elapsed;
        as.runCount++;
        stopDuration = elapsed;
        as.lastStopDuration = stopDuration;
        as.lastRunDuration = 0;
        String reason = "Sim - Unspecified";
        if (!simDowntimeReasons.empty()) {
          int randomIndex = random(0, simDowntimeReasons.size());
          reason = "Sim - " + simDowntimeReasons[randomIndex];
        }
        logEvent(i, true, now_ts, reason.c_str(), 0, stopDuration);
      } else { // Machine just STOPPED (was running)
        as.runningTime += elapsed;
        as.stopCount++;
        runDuration = elapsed;
        as.lastRunDuration = runDuration;
        as.lastStopDuration = 0;
        logEvent(i, false, now_ts, nullptr, runDuration, 0);
      }
      
      as.lastState = !simulatedAssets[i].isRunning;
      as.lastChangeTime = now_ts;
      
      unsigned long nextInterval = random(60000, 600001);
      simulatedAssets[i].nextEventTime = currentTime + nextInterval;
    }
  }

  // --- SIM PATCH: Add System-Wide State Logic for Serial Mode ---
  if (config.monitoringMode == MONITORING_MODE_SERIAL) {
    bool isAnyAssetCurrentlyStopped = false;
    char currentTriggerAssetName[32] = "";
    currentTriggerAssetName[0] = '\0';

    // Determine the current system state based on the simulated assets
    for (int i = 0; i < config.assetCount; i++) {
      if (!simulatedAssets[i].isRunning) {
        isAnyAssetCurrentlyStopped = true;
        strncpy(currentTriggerAssetName, simulatedAssets[i].name.c_str(), 31);
        currentTriggerAssetName[31] = '\0';
        break; // Found the first stopped asset, which triggers the system stop
      }
    }

    // Now, check if this state is different from the global system state and update if needed
    if (isAnyAssetCurrentlyStopped && !g_isSystemSerialDown) {
      // System is transitioning to DOWN
      unsigned long systemUpDuration = now_ts - g_systemLastStateChangeTime;
      g_systemTotalRunningTimeSecs += systemUpDuration;
      g_isSystemSerialDown = true;
      g_systemLastStateChangeTime = now_ts;
      g_systemStopCount++;
      strncpy(g_serialSystemTriggerAssetName, currentTriggerAssetName, 31);
      g_serialSystemTriggerAssetName[31] = '\0';
      logSystemEvent(false, now_ts, g_serialSystemTriggerAssetName);

    } else if (!isAnyAssetCurrentlyStopped && g_isSystemSerialDown) {
      // System is transitioning to UP
      unsigned long systemDownDuration = now_ts - g_systemLastStateChangeTime;
      g_systemTotalStoppedTimeSecs += systemDownDuration;
      g_isSystemSerialDown = false;
      g_systemLastStateChangeTime = now_ts;
...
String htmlConfig() {
  ...
  // --- SIM PATCH --- START: Add simulation control card HTML ---
  html += "      <div class='card shadow-sm mb-4'>";
  html += "        <div class='card-header bg-info text-white'><h4 class='mb-0'>Event Simulation (for Testing)</h4></div>";
  html += "        <div class='card-body'>";
  html += "          <p>Generate random START/STOP events in the background. This will continue to run even if you leave this page and will disable physical pin monitoring.</p>";
  html += "          <div id='simulationNotification' class='alert alert-info' style='display: none;'></div>";
  html += "          <button id='simToggleButton' class='btn btn-primary'>Start Simulation</button>";
  html += "          <hr>";
  html += "          <h5>Simulation Status</h5>";
  html += "          <div id='simulationStatusContainer'><p>Loading status...</p></div>";
  html += "        </div>";
  html += "      </div>";
  // --- SIM PATCH --- END ---
  ...
  html += R"rawliteral(
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // --- SIM PATCH --- START ---
      // Add the new handler for simulation controls
      function handleSimulationControls() {
        const toggleBtn = document.getElementById('simToggleButton');
        const notification = document.getElementById('simulationNotification');
        const statusContainer = document.getElementById('simulationStatusContainer');

        if(!toggleBtn) return; // In case element isn't on the page
        ...
        toggleBtn.addEventListener('click', () => {
            ...
            fetch(endpoint, { method: 'POST' })
                .then(() => setTimeout(updateSimStatus, 1000));
        });
            ...
            fetch('/api/simulation/status')
                ...
                .then(data => {
                    ...
                    if (data.enabled) {
                        ...
                        notification.innerHTML = '<strong>Simulation is running.</strong> Physical pin monitoring is disabled.';
                        ...
                        data.assets.forEach(asset => {
                            statusHtml += `<li class="list-group-item d-flex justify-content-between align-items-center">
                                             ${asset.name}
                                             <span class="badge bg-secondary rounded-pill">Next event in: ${asset.timeToNext}</span>
                                           </li>`;
                        });
                        statusHtml += '</ul>';
                        ...
                        statusContainer.innerHTML = '<p>Simulation is currently stopped.</p>';
                    }
                }).catch(err => {
                    console.error("Could not fetch sim status", err);
                    statusContainer.innerHTML = '<p class="text-danger">Could not fetch simulation status.</p>';
                    toggleBtn.disabled = false;
                });
        }
        setInterval(updateSimStatus, 5000);
        updateSimStatus();
      }
...
void handleApiSummary() {
  ...
  if (config.monitoringMode == MONITORING_MODE_SERIAL) {
    ...
    // --- SIM PATCH: Recalculate live system state if simulation is running ---
    bool isSystemDownNow = false;
    String triggerAssetNow = "";

    if (simulationEnabled) {
      for (int i = 0; i < config.assetCount; i++) {
        if (!simulatedAssets[i].isRunning) {
          isSystemDownNow = true;
          triggerAssetNow = simulatedAssets[i].name;
          break; // Found the first stopped asset
        }
      }
    } else {
      // Use the global state when not simulating
      isSystemDownNow = g_isSystemSerialDown;
      triggerAssetNow = String(g_serialSystemTriggerAssetName);
    }
    
    json += "\"isDown\":" + String(isSystemDownNow ? "true" : "false") + ",";
    json += "\"triggerAsset\":\"" + triggerAssetNow + "\",";
    // --- End SIM PATCH for system state ---
  ...
  for (uint8_t i = 0; i < config.assetCount; ++i) {
    ...
    // --- SIM PATCH: Determine asset state based on simulation mode ---
    bool assetIsCurrentlyRunning;
    if (simulationEnabled) {
      assetIsCurrentlyRunning = simulatedAssets[i].isRunning;
    } else {
