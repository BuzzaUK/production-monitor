...
void loadConfig() {
  if (prefs.isKey("cfg")) { 
    ...
    // PATCH: If updating from an old config, set default if uninitialized
    if (config.longStopThresholdSec == 0) config.longStopThresholdSec = 5*60;
  }
  else {
    ...
    config.tzOffset = 0;
    config.longStopThresholdSec = 5*60; // PATCH: Default 5 minutes
    saveConfig();
  }
}
...
// This function sets up NTP and ensures the ESP uses UK local time including BST.

void setupTime() {
  configTime(0, 0, "pool.ntp.org", "time.nist.gov"); // Start NTP sync
  setenv("TZ", "GMT0BST,M3.5.0/1,M10.5.0/2", 1);     // Set UK time with BST (auto DST switching)
  tzset();                                           // Apply timezone config
...
String htmlDashboard() {
  ...
  html += ".nav .nav-btn{background:#fff;color:#ffa726;border:1.5px solid #ff9800;border-radius:6px;padding:0.7em 1.1em;font-size:1.1em;font-weight:700;box-shadow:0 2px 8px #0001;cursor:pointer;transition:background .2s;text-decoration:none;display:inline-block;}";
  html += ".nav .nav-btn:hover{background:#fff3e0;}";
  ...
  html += ".statrow{display:flex;gap:1em;flex-wrap:wrap;justify-content:flex-start;margin:1.2em 0 1em 0;}";
  html += ".stat{flex:1 1 220px;border-radius:7px;padding:1em;text-align:left;font-size:1.1em;margin:0.3em 0;box-shadow:0 2px 8px #0001;font-weight:500;background:#e6fbe7;border:2px solid #54c27c;min-width:220px;max-width:250px;}";
  ...
  html += "td:last-child .analytics-btn{margin:0;}";
  html += ".analytics-btn{background:#fff;color:#ffa726;border:1.5px solid #ff9800;border-radius:6px;padding:0.45em 1.1em;font-size:1em;font-weight:700;box-shadow:0 2px 8px #0001;cursor:pointer;margin-left:0.6em;transition:background .2s;}";
  html += ".analytics-btn:hover{background:#fff3e0;}";
  ...
  html += "@media (max-width:700px){header{font-size:1.3em;padding:1em 0 1em 1em;}.nav{flex-direction:column;align-items:center;margin:1em 0 1em 0;gap:0.4em;}.card{padding:0.7em;}}";
  html += "</style>";
  html += "</head><body>";
  ...
  fetch('/api/summary').then(r=>r.json()).then(data=>{
    ...
    for(let i=0;i<n;++i){
      ...
      let v0 = asset.name,
          ...
          v7 = asset.stop_count,
          v8 = `<form action='/analytics' method='GET' style='display:inline;'><input type='hidden' name='asset' value="${encodeURIComponent(asset.name)}"><button type='submit' class='analytics-btn'>Analytics</button></form>`;
      let vals = [v0,v1,v2,v3,v4,v5,v6,v7,v8];
      for(let j=0;j<9;++j) row.cells[j].innerHTML = vals[j];
      // Stat card (NO analytics button here)
...
String htmlAnalytics() {
  ...
  if (assetName == "") {
    server.send(400, "text/html", "<h2>No asset specified</h2>"); return "";
  }
  ...
  html += "th,td{text-align:left;}";
  html += "header{background:#1976d2;color:#fff;padding:1.3rem 0;text-align:center;box-shadow:0 2px 10px #0001;font-size:2em;}";
  html += ".nav{display:flex;justify-content:center;gap:1rem;margin:1rem 0;flex-wrap:wrap;}";
  html += ".nav button, .nav a{background:#fff;color:#1976d2;border:none;border-radius:6px;padding:0.7em 1.1em;font-size:1.1em;font-weight:700;box-shadow:0 2px 8px #0001;cursor:pointer;text-decoration:none;display:inline-block;}";
  html += ".nav button:hover, .nav a:hover{background:#e3f0fc;}";
  html += ".nav .analytics-btn{background:#fff;color:#ffa726;border:1.5px solid #ff9800;}";
  html += ".nav .analytics-btn:hover{background:#fff3e0;}";
  html += ".main{max-width:1100px;margin:1rem auto;padding:1rem;}";
  html += ".metrics{display:flex;gap:2em;flex-wrap:wrap;margin-bottom:1.2em;}";
  html += ".metric{background:#f8fafb;border-radius:7px;padding:0.9em 1.5em;font-size:1.2em;margin:0.3em 0;box-shadow:0 2px 8px #0001;font-weight:500;}";
  html += ".controls{display:flex;gap:1em;align-items:center;margin-bottom:1.5em;flex-wrap:wrap;}";
  html += ".controls label{font-weight:500;}";
  html += ".controls input[type='datetime-local']{padding:0.3em;font-size:1em;}";
  html += ".controls .toggle{margin-left:0.5em;}";
  html += ".controls .export-btn{margin-left:0.5em;padding:0.4em 1em;background:#1976d2;color:#fff;border:none;border-radius:4px;cursor:pointer;}";
  html += ".controls .export-btn:hover{background:#135da0;}";
  html += ".chartcard{background:#fff;border-radius:10px;box-shadow:0 2px 10px #0001;margin:1.5em 0;padding:1em;}";
  html += ".tablecard{overflow-x:auto;}";
  html += "@media(max-width:700px){header{font-size:1.3em;}.metrics{gap:1em;flex-direction:column;}.main{padding:0.6em;}.chartcard{padding:0.7em;}}";
  html += "</style></head><body>";
  html += "<header>Analytics: " + assetName + "</header>";
  html += "<nav class='nav'>";
  ...
  html += "<h3>Recent Events</h3><table style='width:100%;'><thead><tr>"
         "<th>Date</th><th>Asset</th><th>Event</th><th>State</th><th>Runtime</th>"
         "<th>Downtime</th><th>Stops</th><th>Note</th></tr></thead>"
         "<tbody id='recentEvents'></tbody></table></div>";
  html += "<script>";
  html += R"rawliteral(
let asset = decodeURIComponent(new URLSearchParams(window.location.search).get("asset") || "");
let allEvents = [];
let downtimeReasons = [];
let longStopThresholdSec = 300;
let eventChart = null;
let filtered = [];
...
function fetchAnalyticsData() {
  fetch('/api/config').then(r=>r.json()).then(cfg=>{
    ...
    longStopThresholdSec = cfg.longStopThresholdSec || 300;
    fetch('/api/events').then(r=>r.json()).then(events=>{
      allEvents = events
        .map(l=>l.split(','))
        .filter(v=>v.length>13 && v[2]===asset);
      setupRangePickers();
      renderEventChart();
      renderRecentEvents();
    });
...
function toDatetimeLocal(dt) {
  // Converts Date to 'YYYY-MM-DDTHH:MM' in local time, for input[type=datetime-local]
...
function setupRangePickers() {
  ...
  let times = allEvents.map(e => new Date(e[0].split('/').reverse().join('-')+'T'+e[1]));
  let max = new Date(Math.max.apply(null, times));
  let defTo = max;
  let defFrom = new Date(defTo.getTime()-12*60*60*1000); // last 12 hours
  document.getElementById('fromTime').value = toDatetimeLocal(defFrom);
  document.getElementById('toTime').value = toDatetimeLocal(defTo);
  document.getElementById('fromTime').onchange = renderEventChart;
  document.getElementById('toTime').onchange = renderEventChart;
  document.getElementById('showStart').onchange = renderEventChart;
  document.getElementById('showStop').onchange = renderEventChart;
...
function renderKPIs(filtered) {
  let kpiDiv = document.getElementById('kpiMetrics');
  if (!filtered.length) {
    kpiDiv.innerHTML = "<div class='metric'>No data</div>";
    return;
  }
  let stops = filtered.filter(e => e[3] == "STOP").length;
  let runtime = parseFloat(filtered[filtered.length - 1][6]);
  let downtime = parseFloat(filtered[filtered.length - 1][7]);
  let avail = parseFloat(filtered[filtered.length - 1][5]);
  let mtbf = parseFloat(filtered[filtered.length - 1][8]);
  let mttr = parseFloat(filtered[filtered.length - 1][9]);
  kpiDiv.innerHTML =
    `<div class='metric'>Stops: <b>${stops}</b></div>
     <div class='metric'>Runtime: <b>${runtime.toFixed(2)} min</b></div>
     <div class='metric'>Downtime: <b>${downtime.toFixed(2)} min</b></div>
     <div class='metric'>Availability: <b>${avail.toFixed(2)}%</b></div>
     <div class='metric'>MTBF: <b>${mtbf.toFixed(2)} min</b></div>
     <div class='metric'>MTTR: <b>${mttr.toFixed(2)} min</b></div>`;
}

function renderEventChart() {
  if (!allEvents.length) return;
  let from = new Date(document.getElementById('fromTime').value);
  let to = new Date(document.getElementById('toTime').value);
  let showStart = document.getElementById('showStart').checked;
  let showStop = document.getElementById('showStop').checked;
  filtered = allEvents
    .map((e,i)=>({...e, idx:i}))
    .filter(e=>{
      let t = new Date(e[0].split('/').reverse().join('-')+'T'+e[1]);
      if (t<from || t>to) return false;
      if (e[3]=="START" && !showStart) return false;
      if (e[3]=="STOP" && !showStop) return false;
      return true;
    });
  renderKPIs(filtered);

  // Format X-axis: show only time if all events are on the same day
  let sameDay = filtered.length > 0 && filtered.every(e => e[0] === filtered[0][0]);
  let times = filtered.map(e => sameDay ? e[1] : (e[0] + " " + e[1]));
  let avail = filtered.map(e=>parseFloat(e[5]));
  let mtbf = filtered.map(e=>parseFloat(e[8]));
  let pointColors = filtered.map(e=>{
    if (e[3]=="STOP" && parseFloat(e[7])>=longStopThresholdSec/60)
      return "#c62828"; // deep red
    if (e[3]=="STOP") return "#ff9800";
    return "#1976d2";
  });
  let pointSizes = filtered.map(e=>{
    if (e[3]=="STOP" && parseFloat(e[7])>=longStopThresholdSec/60)
      return 10;
    if (e[3]=="STOP") return 7;
    return 5;
  });
  ...
  eventChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins:{
        tooltip:{
          callbacks:{
            title:(ctx)=>`Event: ${filtered[ctx[0].dataIndex][3]} at ${ctx[0].label}`,
            label:(ctx)=>{
              const e = filtered[ctx.dataIndex];
              let note = e[13] || "";
              let txt = [
                "State: "+(e[4]=="1"?"RUNNING":"STOPPED"),
                "Runtime: "+parseFloat(e[6]).toFixed(2)+" min",
                "Downtime: "+parseFloat(e[7]).toFixed(2)+" min",
                "Stops: "+e[10]
              ];
              if (e[3]=="STOP" && parseFloat(e[7])>=longStopThresholdSec/60)
                txt.push("Long STOP! ("+parseFloat(e[7]).toFixed(2)+" min)");
              if (note) txt.push("Note: "+note);
              return txt;
            }
          }
        },
      ...
      scales:{
        x:{title:{display:true,text:sameDay ? 'Time' : 'Date/Time'},type:'category'},
        y:{title:{display:true,text:'Availability (%)'},beginAtZero:true},
        y1:{title:{display:true,text:'MTBF (min)'},position:'right',beginAtZero:true,grid:{drawOnChartArea:false}}
      }
    }
  });
}
...
function renderRecentEvents() {
  let tb = document.getElementById('recentEvents');
  tb.innerHTML = "";
  let last10 = allEvents.slice(-10).reverse();
  for (let e of last10) {
    let note = (e[13]||"").replace(/</g,"&lt;").replace(/>/g,"&gt;");
    tb.innerHTML += `<tr>
      <td>${e[0]} ${e[1]}</td>
      <td>${e[2]}</td>
      <td>${e[3]}</td>
      <td>${e[4]=="1"?"RUNNING":"STOPPED"}</td>
      <td>${parseFloat(e[6]).toFixed(2)}</td>
      <td>${parseFloat(e[7]).toFixed(2)}</td>
      <td>${e[10]}</td>
      <td>${note}</td>
    </tr>`;
  }
...
String htmlAnalyticsCompare() {
  ...
  html += "<h3>Overview Table</h3><table style='width:100%;'><thead><tr><th>Asset</th><th>Availability (%)</th><th>Runtime (min)</th><th>Downtime (min)</th><th>Stops</th><th>MTBF (min)</th><th>MTTR (min)</th></tr></thead>";
  html += "<tbody id='compareTable'></tbody></table></div>";
  html += "<script>";
  html += R"rawliteral(
...
function renderCompareTable() {
  ...
  for (let a of allAssets) {
    ...
    tb.innerHTML += `<tr><td>${a}</td><td>${e?parseFloat(e[5]).toFixed(2):"-"}</td><td>${e?parseFloat(e[6]).toFixed(2):"-"}</td><td>${e?parseFloat(e[7]).toFixed(2):"-"}</td><td>${evs.filter(e=>e[3]=="STOP").length}</td><td>${e?parseFloat(e[8]).toFixed(2):"-"}</td><td>${e?parseFloat(e[9]).toFixed(2):"-"}</td></tr>`;
  }
}
fetchCompare();
)rawliteral";
...
String htmlEvents() {
  ...
  html += ".nav button{background:#fff;color:#1976d2;border:none;border-radius:6px;padding:0.7em 1.1em;font-size:1.1em;font-weight:700;box-shadow:0 2px 8px #0001;transition:.2s;cursor:pointer;margin-bottom:0.5em;}";
  html += ".main{max-width:1100px;margin:1rem auto;padding:1rem;}";
  html += ".card{background:#fff;border-radius:10px;box-shadow:0 2px 16px #0002;margin-bottom:1.3rem;padding:1.3rem;}";
...
function renderTable() {
  ...
  for (let i=0; i<displayData.length; ++i) {
    ...
    if (!isMobile) {
      ...
      tr.appendChild(td(Number(lrun).toFixed(2)));
      tr.appendChild(td(Number(lstop).toFixed(2)));
      tr.appendChild(td(Number(lmtbf).toFixed(2)));
      tr.appendChild(td(Number(lmttr).toFixed(2)));
      tr.appendChild(td(stopsInt));
      tr.appendChild(td(levent=="STOP" ? runDur : ""));
      tr.appendChild(td(levent=="START" ? stopDur : ""));
      ...
      card.innerHTML =
        ...
        <div><b>Runtime:</b> ${Number(lrun).toFixed(2)}</div>
        <div><b>Downtime:</b> ${Number(lstop).toFixed(2)}</div>
        <div><b>MTBF:</b> ${Number(lmtbf).toFixed(2)}</div>
        <div><b>MTTR:</b> ${Number(lmttr).toFixed(2)}</div>
        <div><b>Stops:</b> ${stopsInt}</div>
        <div><b>Run Duration:</b> ${(levent=="STOP"? runDur : "")}</div>
        <div><b>Stop Duration:</b> ${(levent=="START"? stopDur : "")}</div>
...
window.onload = fetchChannelsAndStart;
  ...
  html += "<th>Date</th><th>Time</th><th>Asset</th><th>Event</th><th>State</th><th>Avail(%)</th><th>Runtime</th><th>Downtime</th><th>MTBF</th><th>MTTR</th><th>Stops</th><th>Run Duration</th><th>Stop Duration</th><th>Note</th></tr>";
...
String htmlConfig() {
  ...
  html += "</fieldset>";
  // PATCH: Add the long stop threshold input here
  html += "<label>Highlight stops longer than (min): <input type='number' name='longStopThreshold' min='1' max='60' value='" + String(config.longStopThresholdSec/60) + "' required></label>";
...
void handleConfigPost() {
  if (server.hasArg("assetCount")) {
    ...
    // PATCH: Save long stop threshold
    if (server.hasArg("longStopThreshold"))
      config.longStopThresholdSec = constrain(server.arg("longStopThreshold").toInt() * 60, 60, 3600);
