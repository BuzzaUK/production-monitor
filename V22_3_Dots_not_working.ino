...
String htmlAnalytics() {
  ...
  html += "<script>";
  html += R"rawliteral(
// Merged: Updated console log for clarity
console.log('Analytics script started (MergedPatch V1).');

// --- Utility Functions --- (Using user's provided robust versions)
function floatMinToMMSS(val) { 
  ...
  if (totalSeconds >= 3600) { 
    let h = Math.floor(totalSeconds / 3600); let remainingSecondsAfterHours = totalSeconds % 3600;
    let m = Math.floor(remainingSecondsAfterHours / 60); let s = remainingSecondsAfterHours % 60;
...
function toDatetimeLocal(dt) {
  ...
  } catch (e) { const now = new Date(Date.now() - (new Date().getTimezoneOffset() * 60000)); return now.toISOString().slice(0, 16); }
}
...
// Merged: Initialize globalConfig
let globalConfig = { longStopThresholdSec: 300 }; // Default, will be updated

// --- Core Logic ---
function fetchConfigAndEvents() { // Renamed and combined
  if (!asset) {
  ...
  fetch('/api/config') // Merged: Fetch config first
    .then(response => { if (!response.ok) throw new Error(`HTTP error! status: ${response.status} (config)`); return response.json(); })
    .then(cfg => {
      globalConfig.longStopThresholdSec = cfg.longStopThresholdSec || 300;
      return fetch('/api/events'); // Chain to fetch events
    })
    .then(response => { if (!response.ok) throw new Error(`HTTP error! status: ${response.status} (events)`); return response.json(); })
    .then(rawEvents => {
      if (!Array.isArray(rawEvents)) { console.error('Fetched event data is not an array:', rawEvents); allEvents = []; }
      else {
    ...
    .catch(error => {
      console.error('Error fetching config or events:', error);
      const kpiDiv = document.getElementById('kpiMetrics');
...
function renderKPIs(currentFilteredEventsArray) {
  ...
  try {
    ...
    // Merged: Cap availability for KPI display and use .toFixed(1)
    const availability_raw = latestEvent[5] !== undefined ? parseFloat(latestEvent[5]) : 0;
    const availability_capped = Math.min(availability_raw, 100);
    ...
    kpiDiv.innerHTML =
      `<div class='metric'>Stops: <b>${latestEvent[10]}</b></div>
       ...
       <div class='metric'>Availability: <b>${availability_capped.toFixed(1)}%</b></div> 
...
function renderEventChart() {
  if (!allEvents || allEvents.length === 0) { if (eventChart) { eventChart.destroy(); eventChart = null; } const chartCanvas = document.getElementById('eventChart'); if(chartCanvas){const ctx = chartCanvas.getContext('2d'); if(ctx){ctx.clearRect(0,0,chartCanvas.width,chartCanvas.height); ctx.fillText("No data for asset.",chartCanvas.width/2,chartCanvas.height/2);}} return; }
  let fromDate, toDate;
  ...
  filteredEventsGlobal = allEvents.filter(eventRow => {
    try {
      ...
      if ((eventType === "START" || eventType === "SYS_START") && !showStart) return false;
      if ((eventType === "STOP" || eventType === "SYS_STOP") && !showStop) return false;
      return true;
  ...
  try {
    ...
    // Merged: Cap availability data for chart
    let avail = filteredEventsGlobal.map(e => Math.min(parseFloat(e[5] || "0"), 100));
    let mtbfValues = filteredEventsGlobal.map(e => parseFloat(e[8] || "0")); 
    let mttrValues = filteredEventsGlobal.map(e => parseFloat(e[9] || "0"));
    let stateArr = filteredEventsGlobal.map(e => e[4] ? e[4].trim() : 'UNKNOWN_STATE'); 
    
    // Using user's provided point styling logic
    let pointColors = filteredEventsGlobal.map((e, index, arr) => {
      ...
      if (eventType === "STOP" || eventType === "SYS_STOP") { // Logic for STOP event points
        // This logic attempts to find the duration of the stop this event INITIATES.
        // It's complex and relies on looking ahead or specific interpretations of e[12].
        // For this merge, we directly use the user's provided logic.
        if (index + 1 < arr.length) { 
          const nextEvent = arr[index + 1];
          const nextEventType = nextEvent[3] ? nextEvent[3].trim().toUpperCase() : "";
          // User's original logic might misinterpret nextEvent[12] here.
          // Acknowledging user's code for this merge.
          if (nextEventType === "START" || nextEventType === "SYS_START") { durationForStopDecision = nextEvent[12] || "0:0"; } 
          else { durationForStopDecision = e[12] || "0:0"; } // e[12] on a STOP event is not its initiated stop's duration.
        } else { durationForStopDecision = e[12] || "0:0"; }
      }
      // Merged: Use globalConfig.longStopThresholdSec instead of hardcoded 300
      if ((eventType === "STOP" || eventType === "SYS_STOP") && mmssToSeconds(durationForStopDecision) >= globalConfig.longStopThresholdSec) return "#c62828"; 
      if (eventType === "STOP" || eventType === "SYS_STOP") return "#ff9800"; 
      return "#43a047"; // Green for START or SYS_START
    });

    let pointSizes = filteredEventsGlobal.map((e, index, arr) => {
       const eventType = e[3] ? e[3].trim().toUpperCase() : "";
       let durationForStopDecision = "0:0";
       if (eventType === "STOP" || eventType === "SYS_STOP") {
         // Matching logic from pointColors for consistency
         if (index + 1 < arr.length) {
           const nextEvent = arr[index + 1];
           const nextEventType = nextEvent[3] ? nextEvent[3].trim().toUpperCase() : "";
           if (nextEventType === "START" || nextEventType === "SYS_START") { durationForStopDecision = nextEvent[12] || "0:0"; } 
      ...
      let defaultSize = 7; // User's default size
      // Merged: Use globalConfig.longStopThresholdSec
      if ((eventType === "STOP" || eventType === "SYS_STOP") && mmssToSeconds(durationForStopDecision) >= globalConfig.longStopThresholdSec) { defaultSize = 12; } // User's size for long stop
      return defaultSize;
    });
    ...
    let datasets = [{
      ...
      segment: {
        ...
        borderWidth: 3 // User's borderWidth
      }
    }];
    ...
    eventChart = new Chart(ctx, {
      ...
      options: { // User's options structure
        responsive: true, maintainAspectRatio: false,
        interaction: { mode: 'nearest', axis: 'x', intersect: true }, 
        layout: { padding: { top: 15 }}, // User's layout padding
        plugins: {
          tooltip: { /* User's tooltip structure */
            callbacks: {
              title: (tooltipItems) => { /* User's title callback */
                if (!tooltipItems.length) return ''; const idx = tooltipItems[0].dataIndex;
                if (!filteredEventsGlobal[idx]) return 'Error: No data.';
                return `Event: ${filteredEventsGlobal[idx][3]} at ${filteredEventsGlobal[idx][0]} ${filteredEventsGlobal[idx][1]}`;
              },
              label: (tooltipItem) => { /* User's label callback */
                const idx = tooltipItem.dataIndex; const eventRow = filteredEventsGlobal[idx];
                const eventType = eventRow[3] ? eventRow[3].trim().toUpperCase() : "";
                const datasetLabel = tooltipItem.dataset.label || ''; let lines = [];
                if (datasetLabel === 'Availability (%)') {
                  const currentAvail = parseFloat(eventRow[5]).toFixed(2); lines.push(`Availability: ${currentAvail}%`);
                  if (eventType === "START") { const sDS = mmssToSeconds(eventRow[12] || "0:0"); if (sDS > 0) lines.push(`(Prior Stop: ${floatMinToMMSS(sDS / 60.0)})`); }
                  else if (eventType === "STOP") { const rDS = mmssToSeconds(eventRow[11] || "0:0"); if (rDS > 0) lines.push(`(Prior Run: ${floatMinToMMSS(rDS / 60.0)})`); }
                } else if (datasetLabel === 'MTBF' || datasetLabel === 'MTTR') {
                  const cV = tooltipItem.raw; const fCV = floatMinToMMSS(cV); lines.push(`${datasetLabel}: ${fCV}`);
                  if (idx > 0) { /* ... user's change logic ... */ } else { lines.push(`(Initial value)`); }
                  if (datasetLabel === 'MTBF' && eventType === "STOP") { /* ... user's influence logic ... */ }
                  else if (datasetLabel === 'MTTR' && eventType === "START") { /* ... user's influence logic ... */ }
                } return lines.length > 0 ? lines : null;
              }
            }
          },
          legend: { position: 'top' } // User's legend position
        },
        scales: {
          x: { title: { display: true, text: 'Time (HH:MM:SS)' } }, // User's x-axis
          y: { // Availability Axis
            title: { display: true, text: 'Availability (%)' }, 
            beginAtZero: true, 
            // Merged: Use suggestedMax: 120 for visual padding like Image 6
            suggestedMax: 120, 
            // User's ticks callback to hide labels > 100
            ticks: {
                stepSize: 20,    
          ...
          y1: { type: 'linear', display: true, position: 'right', title: { display: true, text: 'MTBF/MTTR' }, beginAtZero: true, grid: { drawOnChartArea: false }, ticks: { callback: val => floatMinToMMSS(val) } } // User's y1-axis
        }
      }
    });
...
function renderRecentEvents() {
  ...
  eventsToDisplay.forEach(eventRow => {
    try {
      if (eventRow.length < 14) { /* malformed row handling */ return; }
      ...
      // Merged: Cap availability for table display and use .toFixed(1)
      tr.insertCell().textContent = parseFloat(Math.min(parseFloat(eventRow[5] || "0"), 100)).toFixed(1); 
      ...
      // Using user's robust formatting for run/stop durations in table
    ...
    } catch (e) { /* error row handling */ }
  });
}
// --- Initialisation ---
// Merged: Call fetchConfigAndEvents
if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', fetchConfigAndEvents); }
else { fetchConfigAndEvents(); }
)rawliteral";
  html += "</script>";
  html += "</div></body></html>";
  return html;
...
function renderTable() {
  ...
  for (let i=0; i<displayData.length; ++i) {
    let vals = displayData[i]; // eventData elements are already arrays from /api/events
    if (!Array.isArray(vals) || vals.length < 14) continue; 
    
    let [ldate,ltime,lasset,levent,lstateVal,lavail,lrun,lstop,lmtbf,lmttr,lsc,runDurStr,stopDurStr] = vals.slice(0,13);
    let lnote = vals.slice(13).join(',').replace(/\n$/, ""); // Handle potential commas in note if CSV was stringified
    let stopsInt = Math.round(Number(lsc));

    // --- Filter for SYSTEM_EVENTS_ONLY ---
    let isSystemEvent = (levent === "SYS_START" || levent === "SYS_STOP");
    if (filterValue === "SYSTEM_EVENTS_ONLY" && !isSystemEvent) {
        continue;
    }
    if (filterValue !== "ALL" && filterValue !== "SYSTEM_EVENTS_ONLY" && lasset !== filterValue) {
        continue;
    }
    // --- End Filter ---
    if (!isSystemEvent && !stateMatcher(lstateVal)) continue; // State filter only for non-system events
    
    let escapedNote = cleanNote(lnote).replace(/"/g, "&quot;").replace(/'/g, "\\'");

    if (!isMobile) {
      let tr = document.createElement('tr');
      if(isSystemEvent) tr.style.backgroundColor = "#e3f2fd"; // Highlight system events
  ...
  scrollInhibit=false; // Reset after use
}
...
String htmlConfig() {
  ...
  html += "input[type=submit].form-button,button.form-button{width:100%;margin-top:1em;padding:0.8em 1.5em;font-size:1.15em;border-radius:8px;border:none;background:#1976d2;color:#fff;font-weight:700;cursor:pointer;} input[type=submit].form-button:hover,button.form-button:hover{background:#1565c0;}";
  ...
  html += "<form method='POST' action='/save_config' id='setupform' onsubmit='setTimeout(showSavedMsg, 500);'>"; // Action changed to /save_config
  ...
  html += "    <p style='font-size:0.9em; color:#555; margin-top:-0.5em; margin-bottom:1em;'>To change the number of assets, update this count and click 'Save All Settings & Reboot'. The page will refresh with the correct number of asset fields.</p>";
  for (uint8_t i = 0; i < config.assetCount; ++i) {
    ...
    html += "      <label>GPIO Pin: <input type='number' name='pin" + String(i) + "' value='" + String(config.assets[i].pin) + "' min='0' max='39' required></label>"; // ESP32 GPIOs typically up to 39
    html += "    </fieldset>";
  }
  html += "  </div>";
  html += "</div>";
