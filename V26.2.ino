...
void setup() {
  ...
  initializeShiftState(); // Initialise Shift Logging
  ...
  server.on("/shiftlogs_page", HTTP_GET, handleShiftLogsPage); // Renamed from /shiftlogs
...
String htmlAssetDetail(uint8_t idx) {
  ...
  html += "      <p class='card-text'><strong>GPIO Pin:</strong> " + String(config.assets[idx].pin) + "</p>";
  html += "      <hr>";
...
String htmlConfig() {
  ...
  // --- Accordion Item: Production Shift Setup ---
  html += "              <div class='accordion-item'>";
  ...
  html += "                </div></div>";
  html += "              </div>";
  // --- END Accordion Item: Production Shift Setup ---
...
function fetchAnalyticsData() {
  ...
  Promise.all([ fetch('/api/events').then(r => { if(!r.ok) throw new Error(`API Events: ${r.status}`); return r.json(); }), fetch('/api/config').then(r => { if(!r.ok) throw new Error(`API Config: ${r.status}`); return r.json(); }) ])
  .then(([rawEvents, configData]) => { 
      ...
      allEvents = Array.isArray(rawEvents) ? rawEvents.filter(e_row => {
          ...
          if (rowLength < 13) { passes = false; reasonForFailure = `Length < 13 (is ${rowLength}).`;}
...
String htmlAnalyticsCompare() {
  ...
  // Asset of day, leaderboard, KPIs, charts, table
  ...
  html += "  <div class='chart-col'><div class='card shadow-sm mb-3'><div class='card-body'><canvas id='barAvail' height='220'></canvas></div></div></div>";
  html += "  <div class='chart-col'><div class='card shadow-sm mb-3'><div class='card-body'><canvas id='barStops' height='220'></canvas></div></div></div>";
  html += "  <div class='chart-col'><div class='card shadow-sm mb-3'><div class='card-body'><canvas id='barMTBF' height='220'></canvas></div></div></div>";
  html += "  <div class='chart-col'><div class='card shadow-sm mb-3'><div class='card-body'><canvas id='barReasons' height='220'></canvas></div></div></div>";
  html += "</div>";
...
function renderLeaderboardPage() {
  ...
  let leaderboard = allAssetNamesCompare.map(e => {
    let av = getLastMetricCompare(byAsset[e], 5);
    return { name: e, availability: av, stops: (byAsset[e]||[]).filter(ev => (ev[3]||"").toUpperCase()=="STOP").length };
  });
...
function renderAssetOfDay() {
  let byAsset = {}; allAssetNamesCompare.forEach(e => { byAsset[e] = [] });
  for (let e of allEventsCompare) byAsset[e[2]] && byAsset[e[2]].push(e);
  let best = allAssetNamesCompare.map(e => ({ name: e, av: getLastMetricCompare(byAsset[e], 5) }))
...
function renderKPICards() {
  let byAsset = {}; allAssetNamesCompare.forEach(e => { byAsset[e] = [] });
  for (let e of allEventsCompare) byAsset[e[2]] && byAsset[e[2]].push(e);
  let topAvail = allAssetNamesCompare.map(e => getLastMetricCompare(byAsset[e], 5)).reduce((a,b)=>Math.max(a,b),0);
  let totalStops = allAssetNamesCompare.map(e => (byAsset[e]||[]).filter(ev => (ev[3]||"").toUpperCase()=="STOP").length).reduce((a,b)=>a+b,0);
  let avgMTBF = allAssetNamesCompare.map(e => getLastMetricCompare(byAsset[e],8)).filter(x=>x>0); avgMTBF = avgMTBF.length? avgMTBF.reduce((a,b)=>a+b,0)/avgMTBF.length:0;
  let html = "";
  html += `<div class='kpi-card'><div class='kpi-icon'><i class="bi bi-trophy-fill" style="color:#ffc107"></i></div><div class='kpi-label'>Top Availability</div><div class='h4 mb-0' id='kpiTopAvail'>${topAvail.toFixed(2)}%</div></div>`;
...
function renderCompareChartsPage() {
  ...
  const PURPLE = 'rgba(111, 66, 193, 0.8)';
  let byAsset = {}; allAssetNamesCompare.forEach(e => { byAsset[e] = [] });
  for (let e of allEventsCompare) byAsset[e[2]] && byAsset[e[2]].push(e);
  ...
  // Chart.js bar gradients (for more playful look)
  function barGradient(ctx, color1, color2) {
  ...
  // Destroy previous
  ["barAvail", "barStops", "barMTBF", "barReasons"].forEach(e => { const t = document.getElementById(e); t && t.chartInstance && t.chartInstance.destroy() });
  const chartOptions = e => ({
    plugins: { title: { display: !0, text: e, font: { size: 16 } }, legend: { display: false } },
    scales: { y: { beginAtZero: !0, ticks: { stepSize: 1, precision: 0 } } },
    responsive: !0, maintainAspectRatio: !1
  });
  if (document.getElementById("barAvail")) {
    let ctx = document.getElementById("barAvail").getContext("2d");
    document.getElementById("barAvail").chartInstance = new Chart(ctx, {
      type: "bar",
      data: { labels: labels, datasets: [{ label: "Availability (%)", data: avail, backgroundColor: avail.map((v,i)=>barGradient(ctx,"#7ded81","#2bcbba")) }] },
      options: chartOptions("Availability by Asset (%)")
    })
  }
  if (document.getElementById("barStops")) {
    let ctx = document.getElementById("barStops").getContext("2d");
    document.getElementById("barStops").chartInstance = new Chart(ctx, {
      type: "bar",
      data: { labels: labels, datasets: [{ label: "Total Stops", data: stops, backgroundColor: barGradient(ctx,"#ffe259","#ffa751") }] },
      options: chartOptions("Total Stops by Asset")
    })
  }
  if (document.getElementById("barMTBF")) {
    let ctx = document.getElementById("barMTBF").getContext("2d");
    document.getElementById("barMTBF").chartInstance = new Chart(ctx, {
      type: "bar",
      data: { labels: labels, datasets: [{ label: "MTBF (min)", data: mtbf, backgroundColor: barGradient(ctx,"#43cea2","#185a9d") }] },
      options: chartOptions("MTBF by Asset (minutes)")
    })
  }
  const brc = document.getElementById("barReasons");
  if (brc) {
    if (reasonLabels.length > 0) {
      let ctx = brc.getContext("2d");
      brc.chartInstance = new Chart(ctx, {
        type: 'bar',
        data: { labels: reasonLabels, datasets: [{ label: 'Event Count', data: reasonData, backgroundColor: barGradient(ctx,"#e0c3fc","#8ec5fc") }] },
        options: chartOptions("Downtime Reason Distribution")
      });
    } else {
      let e = brc.getContext("2d");
      e.clearRect(0, 0, brc.width, brc.height);
      e.textAlign = "center";
      e.fillText("No reason data.", brc.width / 2, brc.height / 2);
    }
  }
}
...
function fetchAndRenderEvents() {
  fetch('/api/events').then(r => { if (!r.ok) throw new Error(`API Events Error: ${r.status}`); return r.json(); })
